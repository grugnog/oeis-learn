...
**Decision**: Parse/type-check, canonicalize, lower, compile and independently verify every admitted final candidate through shared services. Disable optional solvers and regex optimization in strict 007; reject bad proposed assignments even if a solver says success.
...
**Alternatives considered**: Repair all optimization/solver/Rust paths now would delay the first comparison. Merely labeling them unsafe does not prevent accidental use, so strict config rejects them and the runtime adapter is explicit.
...
**Rationale**: Current SFT automatically accepts existing JSON or generates polynomial/Fibonacci/factorial demonstrations. Its codec silently maps unknown constants to UNK. A strict flag on the run name cannot fix these admission paths. Frozen offline pools minimize restart state and avoid repairing the full RL objective before testing representations. The sampler's register/control skeleton is a declared generic prior, not a claim of zero mathematical assumptions.
...
## R7 — Quarantine proofs; defer broader discovery (P0 boundary, later expansion)

**Decision**: Foundation result schemas permit only `proof_status=not_claimed`; the strict path does not call the legacy prover. Keep its known counterexample as a boundary test and preserve finite evidence for later work.

**Rationale**: The prover can substitute a different assumption-bearing symbol from the one in a parsed expression and incorrectly promote a shifted relation. A finite match cannot repair an unsound proof procedure. Full symbolic-domain/certificate design is unnecessary to start the representation experiment.

**Alternatives considered**: Merely adding a warning leaves false proof promotion active. Repairing all theorem/discovery machinery now expands the critical path without improving the language comparison's primary metric.
...
Native LODA operations/numeric limits, cross-language token/primitive trade-offs, comparison stopping rules and practical effect thresholds belong to 008. Fresh evaluation/learning measurements determine later cache, precision, capacity, search, abstraction and proof work. No additional training arms or broad ablation matrix are introduced by 007.
